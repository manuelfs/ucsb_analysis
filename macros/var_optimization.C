...
void opti_var(float cuts[], float fractions[], int nbins, vector<double_double> variable, double totw){
...
void var_optimization(long fixnentries=-1){
  ...
  TString sVariable[] = {"spher", "mt", "dr_bb", "wlep_dphi"};
...
// 	  <<vVariable[var][vVariable[var].size()-1].first<<endl;
      ...
      int n = 10;
      opti_var(cuts, fractions, n, vVariable[var], wVariable[var]);
